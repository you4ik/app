...
var connectionString = 'postgres://default:w9UuYScFEy3M@ep-spring-dream-58410209.eu-central-1.aws.neon.tech:5432/verceldb?sslmode=require';
...
function main() {
  var formatDate, formatOrders, totalItems, totalSum, totalStop, query, res, getOrders, message;
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 2:
            ...
            for (var date in orders) {
              if (Array.isArray(orders[date])) {
                ...
                orders[date].forEach(function (order) {
                  formatted += "- ".concat(order.kol, " @ ").concat(order.sum, " ").concat(order.desc || '', "\n");
                }); // Add a newline between different dates

                formatted += "\n";
              } else {
                console.error("Orders for ".concat(date, " is not an array:"), orders[date]);
              }
            }
          ...
          _context.next = 11;
          return regeneratorRuntime.awrap(client.query(query, [startDate, endDate]));
        ...
        case 16:
          ...
          message = "\n\n".concat(formatOrders(getOrders), "\n\n\n***** TOTAL  *****\n - AMOUNT: ").concat(totalItems(getOrders) + 2, "           \n - SUMMA: ").concat(totalSum(getOrders), ",       \n - STOP: ").concat(totalStop(getOrders), ",         \n*****************\n\n**** BALANCE ****\n - SEK: ").concat(totalSum(getOrders) - totalStop(getOrders), "          \n - USDT: 80            \n - EUR: 0              \n*****************\n"); // Выводим сообщение
